...
#ifdef USING_BUFFER
#define LOAD(tensor, meta, tidx, layout) tensor[div_4(tensor4d_idx_to_linear_idx(meta, tidx))]
#define STORE(tensor, meta, tidx, layout, val) tensor[div_4(tensor4d_idx_to_linear_idx(meta, tidx))] = val
#else
#define LOAD(tensor, meta, tidx, layout) texelFetch(tensor, tensor4d_idx_to_texel_pos_simple(meta, tidx, layout), 0)
#define STORE(tensor, meta, tidx, layout, val) imageStore(tensor, tensor4d_idx_to_texel_pos_simple(meta, tidx, layout), val)
#endif
