...
def escape(line: str) -> str:
    ...
    while "${" in line:
        ...
        if start_pos != 0:
            output_parts.append(
                '"'
                + line[:start_pos].replace("\\", "\\\\").replace('"', '\\"')
                + '"'
            )
        output_parts.append("str(" + line[start_pos + 2 : end_pos] + ")")
        line = line[end_pos + 1 :]
    if line:
        output_parts.append('"' + line.replace("\\", "\\\\").replace('"', '\\"') + '"')
...
def preprocess(
    ...
    # Normalize Windows CRLF to LF so line-end regexes behave consistently.
...
def getShaderInfo(srcFilePath: str) -> ShaderInfo:  # noqa: C901
    ...
    with open(srcFilePath, encoding="utf-8") as srcFile:
        for line in srcFile:
...
def genCppFiles(
    ...
    with open(cpp_src_file_path, "w", encoding="utf-8") as fw:
        fw.write(cpp)
