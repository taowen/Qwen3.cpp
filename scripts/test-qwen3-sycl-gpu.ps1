...
function Invoke-Run {
    ...
    $stdoutPath = Join-Path $OutputDir ("sycl_stdout_run{0}.txt" -f $Index)
    $stderrPath = Join-Path $OutputDir ("sycl_stderr_run{0}.txt" -f $Index)
...
if ([string]::IsNullOrWhiteSpace($Exe)) {
    $Exe = Join-Path $repoRoot "build-sycl-oneapi-ninja-icx\qwen3-cli.exe"
}
...
if ([string]::IsNullOrWhiteSpace($BaselineFile)) {
    $BaselineFile = Join-Path $repoRoot "scripts\baselines\sycl-gpu.stdout.txt"
}
if ([string]::IsNullOrWhiteSpace($OutDir)) {
    $OutDir = Join-Path $repoRoot "runs\sycl-test"
}
...
Write-Host "Running SYCL GPU determinism test..."
Write-Host "Exe: $Exe"
...
$checks = @(
    @{ Name = "SYCL device discovery"; Pattern = "Found\s+\d+\s+SYCL devices:" },
    @{ Name = "Layer offload to GPU"; Pattern = "load_tensors:\s+offloaded\s+\d+/\d+\s+layers to GPU" },
    @{ Name = "SYCL KV cache device"; Pattern = "dev\s*=\s*SYCL\d+" }
)
...
if ($run1Text -cne $run2Text) {
    Write-Host "FAIL: SYCL stdout differs across two identical runs."
...
if ($run1Text -cne $baselineText) {
    Write-Host "FAIL: SYCL stdout text does not match pinned baseline text."
    Write-Host ("stdout logs: {0}, {1}" -f $run1.StdoutPath, $run2.StdoutPath)
...
Write-Host "PASS: SYCL backend is active, deterministic, and matches pinned baseline text."
